...
from utils.loss import LabelSmoothingCrossEntropy, SoftTargetCrossEntropy
from utils.lr_generator import LearningRate
from utils.metrics import EvalNet, Evaluate, final_test, merge,merge_notebook
from utils.trainOneStep import create_train_one_step,create_train_one_step_finetune
from datasets.video_dataset import create_dataset
from utils.config_parser_svt import get_config
from utils.helper import cloud_context_init
from utils.logger import get_logger
from utils.MyLossMonitor import LossMonitor
...
class NetWithLoss(nn.Cell):
    ...
    def construct(self, slow_imgs, fast_imgs,labels, index=None):
        logits= self.net(slow_imgs,fast_imgs,use_head=True)
...
def main(args):
    ...
    model = get_vit_base_patch16_224(cfg=args, no_head=False)
    size = ops.Size()
    ...
    if args.test:
        ...
        if args.local_rank == 0:
            ...
            final_top1 ,final_top5 = merge_notebook(args.save_dir, device_num, device_id)
    ...
    if args.device_target in ["CPU", "GPU"]:
        ...
        train_model = create_train_one_step_finetune(net_with_loss, optimizer)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("SVT Finetune", add_help=False)
    parser.add_argument('--config_file', type=str, default="../models/configs/Kinetics/TimeSformer_divST_8x32_224_finetune.yaml")
