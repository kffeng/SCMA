...
def sgd_lr(base_value,final_value,epochs,niter_per_ep,warmup_epochs=0, start_warmup_value=0):
    ...
    lr1=np.linspace(base_value, base_value, niter_per_ep*3)
    # lrt=np.linspace(base_value, base_value, niter_per_ep*5)
    # schedule = np.concatenate((lr1,lrt))
    lr2=np.linspace(base_value/10, base_value/10, niter_per_ep*1)
    schedule = np.concatenate((lr1, lr2))
    lr3=np.linspace(base_value/100, base_value/100, niter_per_ep*1)
    schedule = np.concatenate((schedule, lr3))
    # lr4=np.linspace(base_value/100, base_value/100, niter_per_ep*(epochs-15))
...
class MultiCropWrapper_Teacher(nn.Cell):
    """
    ...
    def construct(self, x ,**kwargs):
        ...
        for end_idx in idx_crops:
            _out = self.backbone(P.concat(x[start_idx: end_idx]),**kwargs)

        return _out[0], _out[1]

class MultiCropWrapper_Student(nn.Cell):
    """
    Perform forward pass separately on each resolution input.
    The inputs corresponding to a single resolution are clubbed and single
    forward is run on the same resolution inputs. Hence we do several
    ...
    """
    def __init__(self, backbone, vary_fr=False):
        super(MultiCropWrapper_Student, self).__init__()
        # disable layers dedicated to ImageNet labels classification
        if hasattr(backbone, 'fc'):
            backbone.fc, backbone.head = Identity(), Identity()
        self.backbone = backbone
    ...
    def construct(self, x, ids_keep_g=None,ids_mask_g=None,ids_keep_l=None,**kwargs):
        # convert to list
        if not isinstance(x, list):
            x = [x]
        ...
        for end_idx in idx_crops:
            ...
            _out = self.backbone(P.concat(x[start_idx: end_idx]), ids_keep_g=ids_keep_g,ids_mask_g=ids_mask_g,ids_keep_l=ids_keep_l,**kwargs)
            if start_idx == 0:
                output_vis = _out[:,1:]
            else:
                output_cls = _out[:,0]
