...
def final_test(data_loader, model, test_num_spatial_crops, file):
    ...
    acc=0
    for batch in data_loader.create_tuple_iterator():  # batch:<list>[ms.Tensor,ms.Tensor,ms.Tensor,ms.Tensor,]
        acc+=1
        if acc%10==0:
            print(f"{acc}/{data_loader.get_dataset_size()}")
        videos = batch[0]   #(batch, 1, 3, 16, 224, 224)
        target = batch[1]   #(batch, 1)
        names = batch[2].asnumpy()  #(batch,)
        ids = batch[3]          #(batch,)

        chunk_nb = ids // test_num_spatial_crops
        split_nb = ids % test_num_spatial_crops

        logits = model(videos, use_head=True)  #(batch, 400),(batch,)
        ...
        for i in range(logits.shape[0]):
            ...
            final_result.append(string)     #name [400*int] target chunk_nb split_nb
