...
class LossMonitorSVT(Callback):
    ...
    def __init__(self, per_print_times=1, log=None):
        super(LossMonitorSVT, self).__init__()
...
class LossMonitorsvt(Callback):
    ...
    def __init__(self, accum_iter=1, per_print_times=100, ifeval=True,valid_frequency=None, log=None):
        super(LossMonitorsvt, self).__init__()
        if not isinstance(per_print_times, int) or per_print_times < 0:
            raise ValueError("The argument 'per_print_times' must be int and >= 0, "
                             "but got {}".format(per_print_times))
    ...
    def on_train_epoch_end(self, run_context):
        ...
        if self.eval and cur_epoch in self.valid_frequency:
            ...
            self.log.info("Epoch time: %5.3fms per step time: %5.3fms avg loss: %5.3f,metric: %s"%(epoch_mseconds,per_step_mseconds,np.mean(self.losses),str(eval_results)))
        else:
            print(f"Epoch time: {epoch_mseconds:5.3f} ms, "
                  f"per step time: {per_step_mseconds:5.3f} ms, "
                  f"avg loss: {np.mean(self.losses):5.3f}", flush=True)
            self.log.info("Epoch time: %5.3fms per step time: %5.3fms avg loss: %5.3f"%(epoch_mseconds,per_step_mseconds,np.mean(self.losses)))
...
class MyLossMonitor(Callback):
    ...
    def step_end(self, run_context):
        ...
        if self._per_print_times != 0 and (cb_params.cur_step_num - self._last_print_time) >= self._per_print_times:
            ...
            costtime=time.time()-self.time
            self.time=time.time()
            self.log.info("epoch: %s step: %s, loss is %s,time cost is %.2fs" % (cb_params.cur_epoch_num, cur_step_in_epoch, loss,costtime))
...
class LossMonitor(Callback):
    ...
    def __init__(self, accum_iter=1, per_print_times=1, ifeval=True,valid_frequency=None, log=None):
