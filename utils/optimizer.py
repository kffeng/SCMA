...
def get_eval_optimizer(net, steps_per_epoch, args):
    lr = get_lr(lr_init=0.008, lr_end=1e-5, lr_max=0.008,
                warmup_epochs=0, total_epochs=args.epoch_size,
                steps_per_epoch=steps_per_epoch,
                lr_decay_mode="cosine")
    lr = Tensor(lr)
    # output = nn.cosine_decay_lr(min_lr=0.0, max_lr=0.001, total_step=args.epoch_size*steps_per_epoch, step_per_epoch, decay_epoch)
    optimizer = nn.SGD(params=net.trainable_params(), learning_rate=lr,momentum=0.9)
    return optimizer
