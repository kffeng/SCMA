...
class Attention(nn.Cell):
    ...
    # @ms_function
    def construct(self, x, B, N, C, return_attn=False):
        if self.with_qkv:
...
class Block(nn.Cell):
    ...
    def construct(self, x, B, T, W, return_attn=False):
        ...
        if self.attention_type in ['space_only', 'joint_space_time']:
            ...
            res_temporal = self.drop_path(self.temporal_attn(F.cast(self.temporal_norm1(xt), self.dtype), A, N, C))
            ...
            if return_attn:
                D, E, G = xs.shape
                _, attn = self.attn(self.norm1(xs), D, E, G, return_attn=return_attn)
                return attn
            else:
                D, E, G = xs.shape
                xs = xs.view(-1, self.dim)
                res_spatial = self.drop_path(self.attn(F.cast(self.norm1(xs), self.dtype), D, E, G))
            res_spatial = res_spatial.view(D, E, G)
            ...
            #     x = ms.ops.concat((init_cls_token, x, init_aux_cls_token), axis=1) + ms.ops.concat(
            #         (cls_token, res, aux_cls_token), axis=1)
            V, L, P = x.shape
            x = x.view(-1, self.dim)
            x = x + self.drop_path(self.mlp(F.cast(self.norm2(x), self.dtype)))
            x = x.view(V, L, P)
...
class VisionTransformer(nn.Cell):
    ...
    def forward_features(self, x, ids_keep=None, get_all=False, get_attn=False):
        # print(x.shape)  # (4, 3, 8, 224, 224)
        B = x.shape[0]
        x, T, W = self.patch_embed(x)  # 出来后  x-->(32, 196, 768)  bs=2  32=16*2     T=16  W=14
        # print("333333")
        # print(B)
        _,_,C=x.shape
        # print("11111111")
        ...
        if x.shape[1] != self.pos_embed.shape[1]:
            ...
            # new_pos_embed = new_pos_embed.view(1, x.shape[2], -1)
        ...
        #块状掩码
        if self.masked_im_modeling and B!=8:
            W=7
            cls_t = ms.ops.expand_dims(x[:, 0, :], 1)
            
            x = x[:, 1:, :]   # 128,196,768
            _,N,T,L=ids_keep.shape     #2,8,8,64  
            ids_keep=ids_keep.reshape(-1,L)
        ...
        if self.attention_type != 'space_only':
            ...
            M = x.shape[-1]
            x = x.reshape(-1, T, M)  # [16,196,768]   --->    [392,8,768]
            # Resizing time embeddings in case they don't match
            if T != self.time_embed.shape[1]:

                time_embed = self.time_embed.transpose(0, 2, 1)
                time_embed = P.ExpandDims()(time_embed, 3)
                new_size = (T, 1)
                new_time_embed = P.ResizeNearestNeighbor(new_size)(time_embed)
                new_time_embed = P.Squeeze(3)(new_time_embed)
        ...
        if self.attention_type == 'space_only':
            ...
            x = ms.Tensor(np.mean(x, 1, True))
        x = self.norm(x)
        if get_all:
            return x
        return x[:, 0]

    def construct(self, x, ids_keep=None,use_head=False, linear=False):
...
def _conv_filter(state_dict, patch_size=16):
    ...
    for k, v in state_dict.items():
        if 'patch_embed.proj.weight' in k:
            ...
            v = ms.Parameter(v, name="patch_embed.proj.weight", requires_grad=True)
        out_dict[k] = v
    return out_dict
...
def get_vit_base_patch16_224(cfg, no_head=False, masked_im_modeling=False, **kwargs):
    ...
    vit = VisionTransformer(img_size=cfg.TRAIN_CROP_SIZE, num_classes=cfg.NUM_CLASSES,
                            ...
                            qkv_bias=True, norm_layer=nn.LayerNorm, drop_rate=0.,
                            attn_drop_rate=0., drop_path_rate=0.1, num_frames=cfg.NUM_FRAMES,
...
#         cls_tokens = self.cls_token.broadcast_to((x.shape[0], -1, -1))
#         # cls_tokens = self.cls_token.broadcast_to(x.size(0), -1, -1)
#         x = self.cat((cls_tokens, x))
#         # x = torch.cat((cls_tokens, x), dim=1)
...
#             # new_pos_embed = ms.ops.interpolate(other_pos_embed, sizes=(H, W), mode="bilinear")
#             new_size = (H, W)
#             new_pos_embed = self.fast_nearst_HW(other_pos_embed)
...
#             _,N,T,L=ids_keep.shape     #2,8,8,64  
#             ids_keep=ids_keep.reshape(-1,L)
#             x_vis = x.gather_elements(dim=1, index=ids_keep.expand_dims(-1).repeat(768, axis=-1))
#             x = self.cat((cls_t, x_vis))
...
#                 new_size = (T, 1)
#                 new_time_embed = self.fast_nearst_T(time_embed)
...
#     # def construct(self, x1, x2, use_head=False, linear=False):
#     #     if linear:
#     #         x1 = self.forward_features(x1)
#     #         x2 = self.forward_features(x2)
#     #         x = (x1 + x2)/ms.Tensor(2.0)
#     #     else:
#     #         x1 = self.forward_features(x1)
#     #         x2 = self.forward_features(x2)
#     #         x = (x1 + x2)/ms.Tensor(2.0)
...
#                             attn_drop_rate=0.0, drop_path_rate=0.1, num_frames=cfg.NUM_FRAMES,
#                             attention_type=cfg.ATTENTION_TYPE,masked_im_modeling=masked_im_modeling,**kwargs)
#     vit.attention_type = cfg.ATTENTION_TYPE
#     vit.default_cfg = default_cfgs['vit_base_patch16_224']
#     vit.num_patches = (cfg.TRAIN_CROP_SIZE // patch_size) * (cfg.TRAIN_CROP_SIZE // patch_size)
