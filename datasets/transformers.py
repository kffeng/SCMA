...
#         return zoom(images, (1, 1, new_height / images.shape[2], new_width / images.shape[3]), order=1)
...
#         self.random_resized_crop = RandomResizedCrop(size=224, scale=self.local_crops_scale, interpolation="bilinear")
#         self.normalize = Normalize()
#         self.flip_and_color_jitter = FlipAndColorJitter()
...
#         images = [x.astype(float) / 255.0 for x in images]
#         for i in images:
#             frames = self.random_resized_crop(i)
#             frames = self.flip_and_color_jitter(frames)
#             frames = self.normalize(frames)
#             result.append(frames)
#         return np.array(result)
...
# class Change:
#     def __call__(self, frames):
#         frames = [x.transpose(1, 0, 2, 3) for x in frames]
#         return np.array(frames)
...
class TransposeReshape_ms:
    def __init__(self, repeat_aug=1):
        self._repeat_aug = repeat_aug
    
    def __call__(self, frames):
        T,C,H,W = frames[0].shape
        frames = [x.transpose(2, 3, 0, 1) for x in frames]   #h,w,t,c
        # frames = frames.transpose(2, 3, 0, 1) # h, w, r, c, t
        frames = [x.reshape(H, W,T*C) for x in frames]
        # frames = frames.reshape(H, W,C*T)
        return frames
...
#         # self.col_norm=ColorNormalization_ms()

    
#     def __call__(self, frames):
#         frames = [x.transpose(2,0,1) for x in frames]
#         c, h, w = frames[0].shape
#         frames = [x.reshape(self._num_frames, 3,h, w) for x in frames]
#         # frames = [self.col_norm(x) for x in frames]
#         # frames = frames.reshape(3, self._num_frames, h, w) # r, c, t, h, w
#         frames=[x.astype(np.float32) for x in frames]
#         # frames=frames.astype(np.float32)
#         # frames = frames.transpose(1, 0, 2, 3, 4) # r, c, t, h, w
#         return frames
...
class ReshapeTranspose_ms:
    ...
    def __call__(self, frames):
        ...
        c, h, w = frames.shape
        frames = frames.reshape(self._num_frames,3 ,h, w) # r, c, t, h, w
...
class FlipAndColorJitter_ms:
    ...
    def __call__(self, frames):
        frames= self.horizontal(frames)
        frames=self.reshapeTranspose_ms(frames)
        if np.random.uniform() < 0.8:
            frames = self.color_jitter(frames)
        if np.random.uniform() < 0.2:
            frames = self.grayscale(frames)
...
class GlobalTransform_ms:           
    def __init__(self, global_crops_scale=(0.4, 1.0)):
        self.global_crops_scale = global_crops_scale
        self.random_resized_crop = vision.RandomResizedCrop(size=224, scale=(0.4,1.0), ratio=(3. / 4., 4. / 3.))
